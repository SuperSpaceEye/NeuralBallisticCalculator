...
inline int64_t time_in_air(double y0, double y, double Vy, int64_t max_steps=100000) {
    int64_t t = 0;
...
rough_pitch_estimation(const std::array<int64_t, 3> & cannon,
                       const std::array<int64_t, 3> & target,
                       double distance,
                       int64_t initial_speed,
                       int64_t length,
                       int max_steps) {
...
inline std::vector<std::pair<double, double>>
py_rough_pitch_estimation(pybind11::tuple & cannon_t,
                       pybind11::tuple & target_t,
                       double distance,
                       int64_t initial_speed,
                       int64_t length,
                       int max_steps) {
    std::array<int64_t, 3> cannon{
            cannon_t[0].cast<int64_t>(),
            cannon_t[1].cast<int64_t>(),
            cannon_t[2].cast<int64_t>(),
    };
    std::array<int64_t, 3> target{
            target_t[0].cast<int64_t>(),
            target_t[1].cast<int64_t>(),
            target_t[2].cast<int64_t>(),
    };
...
fine_pitch_estimation(const std::array<int64_t, 3> & cannon,
                      const std::array<int64_t, 3> & target,
                      double distance,
                      int64_t initial_speed,
                      int64_t length,
    ...
    std::vector<std::array<double, 3>> delta_times;
    auto pitches = linspace<double>(pitch-1, pitch+1, num_refined);
    for (auto & tried_pitch: pitches) {
        ...
        delta_times.push_back({delta_t, tried_pitch, (double)time_air});
    }
    return delta_times;
}

inline std::vector<std::array<double, 3>>
py_fine_pitch_estimation(pybind11::tuple & cannon_t,
                      pybind11::tuple & target_t,
                      double distance,
                      int64_t initial_speed,
                      int64_t length,
...
inline std::array<double, 3>
            try_pitch(const std::array<int64_t, 3> & cannon,
                      const std::array<int64_t, 3> & target,
    ...
    if (delta_times1.empty()) {return std::array<double, 3>{-1, -1, -1};}
    ...
    if (delta_times2.empty()) {return std::array<double, 3>{-1, -1, -1};}
...
inline std::array<double, 3>
py_try_pitch(pybind11::tuple & cannon_t,
          pybind11::tuple & target_t,
          long power, long length, int max_steps) {
    std::array<int64_t, 3> cannon{
            cannon_t[0].cast<int64_t>(),
            cannon_t[1].cast<int64_t>(),
            cannon_t[2].cast<int64_t>(),
...
auto threaded_make_dataset(std::vector<std::array<std::array<double, 3>, 2>> * dataset,
                           int64_t charges,
                           int length,
    ...
    for (int y = start_pos; y < max_height_difference/2; y+=num_threads) {
        ...
        while (true) {
            ...
            if (res[0] < 0) {
                ...
                dataset->push_back(std::array<std::array<double, 3>, 2>{
                        std::array<double, 3>{(double )x, (double)y, 0},
                        res
                });
                x++;
            }
...
auto make_dataset(int64_t charges,
                  int length,
    ...
    std::vector<std::vector<std::array<std::array<double, 3>, 2>>> threads_result;
    ...
    while (not_ended) {
        ...
        for (auto & holder: threads) {
            if (holder.wait_for(0ms) != std::future_status::ready) {
                not_ended += true;
                break;
            }
        }
